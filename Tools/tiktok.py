...
class Main():
    try:
        def download(url):
            ...
            global x, videocount
            try:
                with open(f'./downloaded/tiktok{random.randint(1000, 9999)}.mp4', 'wb') as out_file:
                    videobytes = requests.get(f'{url}.mp4', stream=True)
                    out_file.write(videobytes.content)
                    x += 1
                    os.system(f"title Tiktok Downloader | Downloaded: {x} ^| Folder: ./downloaded")
            except:
                pass
        ...
        def start():
            ...
            os.system(f"title Tiktok Downloader")

            Anime.Fade(Center.Center("Tiktok Downloader"), Colors.blue_to_red, Colorate.Vertical, interval=0.01, time=2)
            Anime.Fade(Center.Center("Input username and the video output named by video id."), Colors.blue_to_red, Colorate.Vertical, interval=0.01, time=5)
